  ...
  try {
    ...
    if (capsule.ownerId.toString() !== session.user.id) {
      return new NextResponse(
        "Forbidden: Only the capsule owner can manage collaborators.",
        { status: 403 }
      );
    }
    ...
    if (!userToAdd) {
      return new NextResponse(
        "User not found. They must sign up to collaborate.",
        { status: 404 }
      );
    }
    ...
    if (capsule.recipientEmails.includes(email.toLowerCase())) {
      return new NextResponse(
        "This user is a recipient. They cannot be added as a collaborator.",
        { status: 400 }
      );
    }

    if (capsule.collaborators.includes(userToAdd._id)) {
      return new NextResponse("User is already contributing to this capsule.", {
        status: 400,
      });
    }
